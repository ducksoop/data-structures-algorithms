...
namespace Algorithms.Lib.SymbolTables
{
    public class BinarySearchSt<TKey, TValue>
    {
        ...
        public void Remove(TKey key)
        {
            ...
            int r = Rank(key);
            if (r == Count || _comparer.Compare(_keys[r], key) != 0) return;
        ...
        {
            int r = Rank(key);
            if (r < Count && _comparer.Compare(_keys[r], key) == 0) return true;
        ...
        {
            var keysTemp = new TKey[newCapacity];
            var valuesTemp = new TValue[newCapacity];
        ...
        {
            if (index < 0) throw new ArgumentOutOfRangeException($"Cannot select since index is out of range.");
        ...
        {
            if (key == null) throw new ArgumentNullException($"Argument to {nameof(Ceiling)} is null.");

            int r = Rank(key);
            if (r == Count) return default;
            else return _keys[r];
        }

        public TKey Floor(TKey key)
        {
            if (key == null) throw new ArgumentNullException($"Argument to {nameof(Floor)} is null.");
        ...
        public IEnumerable<TKey> Range(TKey left, TKey right)
        {
            ...
            if (Contains(right)) q.Enqueue(_keys[Rank(right)]);
